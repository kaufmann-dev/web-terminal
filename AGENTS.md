...
- The Nixpacks start command must create `TERMINAL_WORKDIR` idempotently and pass it to `ttyd` as `--cwd`. Its default is `/code`; keep that default synchronized with `.env.example` and the README storage instructions.
