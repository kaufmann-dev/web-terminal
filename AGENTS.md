...
- Serve only the pinned xterm browser module, fit and WebGL addons, stylesheet, and self-hosted font
  files through explicit same-origin vendor routes. Use WebGL custom glyphs with automatic DOM
  fallback for terminal block and Braille characters. Do not copy package artifacts into the
...
- `nixpacks.toml` owns the immutable system package list and invokes the Git Wrangler, Nixpacks,
  and rootless-Podman image setup scripts. Keep Node's version in `package.json`; do not add
  `NIXPACKS_NODE_VERSION` or a second Node pin.
...
- Keep Nixpacks pinned to v1.41.0 through its checksum-verified AMD64/ARM64 build script because
  v1.40 cannot resolve Node.js 24; keep it out of the Nix package list. Install uv as a Nix system
  package. Install Podman, uidmap, fuse-overlayfs, and pasta through Apt for the nested rootless
  runtime; keep Podman out of the Nix package list.
...
- `scripts/start.sh` validates and creates terminal paths, performs the one-time UID/GID 1000
  ownership migration, validates nested rootless Podman, applies chezmoi as the terminal user, and
  then drops to that user before replacing itself with `node app.js`. The Node shutdown path must
  close WebSockets and terminate all child PTYs.
...
- Validate the Nixpacks plan and generate its build context with `nixpacks build --out`; build and
  run that context with the bundled rootless Podman, never Docker.
